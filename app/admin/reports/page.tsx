...
const AdminReportsPage: React.FC = () => {
  ...
  // Fetch reports from the reports API
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <div>
            <Link href="/admin/reports/new" className="btn-primary">
              Create New Report
            </Link>
          </div>
        </div>
        ...
        <div className="retro-container mb-6">
          <div className="retro-header">Reports List</div>
          <div className="p-4">
            {reports.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {reports.map((report) => (
                      <tr key={report.id} className="border-b border-[#0e3d64]">
                        <td className="p-2">
                          <Link href={`/reports/${report.slug}`} className="text-[#0e3d64] hover:text-[#0a2d4a]">
                            {report.title}
                          </Link>
                        </td>
                        <td className="p-2">{formatDate(report.publishedAt)}</td>
                        <td className="p-2">
                          <div className="flex space-x-2">
                            <Link href={`/admin/reports/edit/${report.slug}`} className="text-[#0e3d64] hover:text-[#0a2d4a]">
                              Edit
                            </Link>
                            <button
